...
"""
Inspired by Ice Net's data loader, a custom data loader class which produces batches of network input/ outputs for training.
See: https://www.tensorflow.org/api_docs/python/tf/keras/utils/Sequence
*SPECIFIC SECTION CLEARLY MARKED - SPECIFICALLY RELATES TO LOADING DATA FROM FILES.*

That said, this code implements the Tensorflow Sequence class and is *commonly* employed during neural network tasks in which
data size exceeds the capacity of RAM. 
"""
...
class ThesisDataLoader(tf.keras.utils.Sequence):
    ...
    def __init__(self, dataset_config):
        """
        Constructur variable, in which data loader configs determine key parameters such as file locations, date ranges and batch sizes.
        """
    ...
    def get_samples(self, date_list):
        ...
        for ID, date in enumerate(date_list):
            ...
            # THIS BLOCK IS INSPIRED BY TOM ANDERSON'S ICE NET - SPECIFICALLY USING CHANNELS & NP.STACK
            # https://github.com/tom-andersson/icenet-paper/blob/main/icenet/utils.py
            channels = 0
